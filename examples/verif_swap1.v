...
Definition swap1_spec :=
  DECLARE _swap1
   ...
   PROP(  )
   LOCAL()
   SEP ((data_at Tsh (tarray (Tunion _sslval noattr) 1) [(inr (c : val))] (x : val)); (data_at Tsh (tarray (Tunion _sslval noattr) 1) [(inr (b : val))] (z : val)); (data_at Tsh (tarray (Tunion _sslval noattr) 1) [(inr (q : val))] (t : val)); (data_at Tsh (tarray (Tunion _sslval noattr) 1) [(inl ((Vint (Int.repr 41)) : val))] (y : val))).


Definition Gprog : funspecs :=
  ltac:(with_library prog [swap1_spec]).

Lemma body_swap1 : semax_body Vprog Gprog f_swap1 swap1_spec.
Proof.
start_function.
ssl_open_context.
