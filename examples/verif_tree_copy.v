...
Definition tree_copy_spec :=
  DECLARE _tree_copy
   ...
   PROP(  )
   LOCAL()
   SEP ((data_at Tsh (tarray (Tunion _sslval noattr) 1) [(inr (y : val))] (r : val)); (tree (y : val) (s : list Z) (a : tree_card)); (tree (x : val) (s : list Z) (a : tree_card))).
...
Exists nullval.
ssl_entailer.
ssl_rewrite_last (unfold_tree_card_0 ).
ssl_entailer.
ssl_rewrite_last (unfold_tree_card_0 ).
ssl_entailer.
...
Exists (y3 : val).
ssl_entailer.
ssl_rewrite_last (unfold_tree_card_2 (_alpha_514x2 : tree_card) (_alpha_513x2 : tree_card)).
Exists (vx22 : Z).
Exists (s2x2 : list Z).
Exists (rx22 : val).
Exists (s1x2 : list Z).
